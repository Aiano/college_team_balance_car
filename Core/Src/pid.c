...
void pid_compute() {
    ...
    Z += MPU6050.Gz * 0.01; // 对时间积分

    /* 读取编码器速度 */
    encoder_get_speed(&motor1, &motor2);
    ...
    u1 = pid_get_u(&pid_stand, target_degree, MPU6050.KalmanAngleX);
    //if (u1 < 50 && u1 > -50) u1 = 0;
    u2 = pid_get_u(&pid_speed, target_speed, (motor1 + motor2) / 2);
    u3 = pid_get_u(&pid_spin, target_angular_speed, motor1 - motor2);

    motor1_set_value((int16_t) (u1 + u2 + u3));
    motor2_set_value((int16_t) (u1 + u2 - u3));

    printf("%d,%d,%d,%d\n", (int) MPU6050.KalmanAngleX, (int) target_degree, (int) (u1 + u2), (motor1 + motor2) / 2);
}
